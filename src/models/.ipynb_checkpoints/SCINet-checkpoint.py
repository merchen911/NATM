...
class pl_SCINet(pl.LightningModule): 
    def __init__(self, 
        ...
        self.metric = SMAPE()
        self.model = SCINet(
            output_len = self._config.output_length,
            input_len = self._config.input_length,
            input_dim = self._config.input_feature,
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        logits = self(x)
        loss = self.criterion(logits, y.squeeze()) 
        mape = self.metric(logits, y.squeeze())

        metrics = {'val_metric': mape, 'val_loss': loss}
        self.log_dict(metrics)
        return metrics

    
    def validation_step_end(self, val_step_outputs):
        val_met  = val_step_outputs['val_metric'].cpu()
        val_loss = val_step_outputs['val_loss'].cpu()

        self.log('val_metric',  val_met, prog_bar=self._config.prog_bar)
        self.log('val_loss', val_loss, prog_bar=self._config.prog_bar)
